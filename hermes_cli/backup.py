        ...
        for member in members:
            ...
            if member.startswith(_EXTERNAL_PREFIX):
                ...
                try:
                    ...
                    # _PRIVATE_RESTORE_FILE_NAMES (fork superset: secrets +
                    # state.db) — upstream wrote _SECRET_FILE_NAMES here against
                    # its own {.env, auth.json, state.db} def, which the merge
                    # dropped in favor of the fork's authoritative secret set.
                    if target.suffix in {".json", ".env", ".conf"} or target.name in _PRIVATE_RESTORE_FILE_NAMES:
                        try:
...
def _create_quick_snapshot_locked(
    ...
    for rel in _QUICK_STATE_FILES:
        ...
        if src.is_dir():
            ...
            # Fork (PR#70) walks with os.walk(followlinks=False) rather than
            # rglob so excluded directories are PRUNED from the traversal and
            # symlinks are never followed out of the hermes root; upstream
            # v0.20 added the max_file_size cap plus failed/oversized DB
            # tracking. Both survive here: exclusion is evaluated FIRST, so a
            # secret-bearing file is never size-tested, copied, or recorded.
