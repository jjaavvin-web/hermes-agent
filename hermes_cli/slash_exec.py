...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    ``ctx.options["platform"]`` is an optional surface parameter: when a
    concrete platform is supplied, ``gateway_help_lines`` hides platform-scoped
    intake commands (``/sol`` is Discord-only) from the listing. Omitting it
    keeps the unfiltered listing for CLI/TUI callers.
    """
    from agent.i18n import t
    from hermes_cli.commands import gateway_help_lines

    platform = ctx.options.get("platform") if ctx.options else None
    lines = [
...
def _exec_commands(ctx: CommandContext) -> CommandReply:
    ...
    ``ctx.options["platform"]`` is optional and scopes the listing the same way
    :func:`_exec_help` does (Discord-only ``/sol`` stays hidden elsewhere).
    """
    from agent.i18n import t
    from hermes_cli.commands import gateway_help_lines
    ...
    entries = list(gateway_help_lines(platform=(ctx.options.get("platform") if ctx.options else None)))
    try:
        from agent.skill_commands import get_skill_commands
        skill_cmds = get_skill_commands()
        if skill_cmds:
