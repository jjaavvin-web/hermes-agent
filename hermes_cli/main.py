...
if str(PROJECT_ROOT) == _project_root_str_fast():
    # Normal source/editable install: the fork guard agreed with the fast
    # path, so use upstream's helper — it also dedupes realpath-equivalent
    # entries already on sys.path before inserting the root at position 0.
    _ensure_project_root_on_path_fast()
else:
    # Fork guard FIRED: __file__ lives in site-packages and a real source
    # checkout was found above. Prefer the checkout — upstream's helper would
    # re-derive the stale site-packages root from ``__file__`` and undo it.
...
# Dispatcher specialist workers arrive with a deliberately minimal environment.
# Do not repopulate it from profile/project/managed secret sources during CLI
# bootstrap; doing so would undo the spawn boundary before tool policy loads.
...
def cmd_chat(args):
    ...
    specialist_authority = bool(
        os.environ.get("HERMES_WORKER_AUTHORITY", "").strip()
    )
    ...
    if not specialist_authority and _termux_should_prefetch_update_check():
        try:
    ...
    # Skipped entirely under specialist authority (a delegated kanban
    # worker/subprocess) — it inherits the parent launch's already-synced
    # skills and doesn't need to redo the scan.
    if not specialist_authority:
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "dump", "egress", "fallback", "gateway", "git-health", "hooks", "import",
...
def _prepare_agent_startup(args) -> None:
    ...
    # Specialist workers must not execute user/project plugin code, launch
    # configured MCP subprocesses, or register shell hooks before the role
    # authority boundary is active. Their constrained built-ins are discovered
