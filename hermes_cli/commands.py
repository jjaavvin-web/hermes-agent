...
#   - sol: fork-local Discord→Sol-kanban intake; a Discord-surface command
#     that has no Slack audience. Kept off the native list so it cannot
#     displace an upstream command at the 50-cap (fork addition; upstream's
#     "credits"/"billing" entries folded into "topup" above).
#   - project: fork-local Telegram→kanban project intake; same rationale as
#     sol — a messaging-surface intake command with no Slack audience.
_SLACK_VIA_HERMES_ONLY = frozenset({
    "topup", "moa", "debug", "egress", "init", "version", "diff", "update",
    "heartbeat", "refine", "pause",
    "sol", "project",
})
