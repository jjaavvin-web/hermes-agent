    ...
    *,
    ...
    while True:
        ...
        if gate is not None and getattr(gate, "ran", False) and not getattr(gate, "passed", False):
            ...
            # The kanban worker loop has no wait-barrier concept (workers
            # finish via kanban_complete / kanban_block, not by parking), so a
            # WAIT verdict is treated as CONTINUE here.
