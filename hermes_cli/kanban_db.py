    ...
    *,
    ...
    with write_txn(conn, allow_nested=True):
        ...
        if expected_run_id is not None and (
            ...
            raise ValueError(
                f"stale run {expected_run_id} cannot comment on active task "
                f"{task_id}"
            )
        cur = conn.execute(
...
def _board_for_connection(conn: sqlite3.Connection) -> str:
    """Resolve the board slug from the connection's canonical DB path."""
    row = conn.execute("PRAGMA database_list").fetchone()
    db_path = Path(row[2]).resolve() if row and row[2] else None
    if db_path is not None:
        active_slug = get_current_board()
        try:
            if kanban_db_path(active_slug).resolve() == db_path:
                return active_slug
        except OSError:
            pass
        try:
            candidates = list_boards(include_archived=True)
        except Exception:
            candidates = []
        if not any(
            (entry.get("slug") or DEFAULT_BOARD) == DEFAULT_BOARD
            for entry in candidates
        ):
            candidates.append(read_board_metadata(DEFAULT_BOARD))
        for entry in candidates:
            slug = entry.get("slug") or DEFAULT_BOARD
            try:
                if kanban_db_path(slug).resolve() == db_path:
                    return slug
            except OSError:
                continue
...
def complete_task(
    ...
    *,
    ...
    # Central specialist boundary: direct kernel/CLI/bridge callers inherit the
    # same dispatcher-pinned task and board authority as public tool dispatch.
    # Admin/internal callers remain unrestricted when no worker role is present.
    if os.environ.get("HERMES_WORKER_AUTHORITY"):
        try:
            from hermes_cli.worker_authority import authorize_current_worker

            authority_args: dict[str, Any] = {"task_id": task_id}
            if os.environ.get("HERMES_KANBAN_BOARD"):
                authority_args["board"] = _board_for_connection(conn)
            authority = authorize_current_worker(
                "kanban_complete",
                authority_args,
            )
...
def block_task(
    ...
    * ``transient`` — treated like a generic block for routing, but a worker
    ...
    """
    # Apply the same task/board pin to direct bridge and kernel block calls.
    if os.environ.get("HERMES_WORKER_AUTHORITY"):
        try:
            from hermes_cli.worker_authority import authorize_current_worker
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    if kind == "worktree":
        try:
            ...
            # Fork fallback: boards without a ``default_workdir`` anchor
            # (e.g. the live hermes board) must not fail every no-path
            # worktree card — anchor on the deterministic kanban worktree
            # root instead (not the dispatcher's CWD).
            branch = task.branch_name or f"kanban/{task.id}"
            if task.workspace_path:
                p = Path(task.workspace_path).expanduser()
                if not p.is_absolute():
                    # Keep the confused-deputy rail: relative worktree
                    # paths are rejected by BOTH sides — never fall back
                    # around this guard.
                    raise
            else:
                p = _kanban_worktree_root() / task.id
            # Create the worktree off the live HEAD so the code→PR layer has
            # a real branch+commits to push. Best-effort — the kanban-worker
            # skill still documents the worker-side `git worktree add`
            # fallback if this no-ops.
...
def heartbeat_worker(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        # Automatic runtime activity remains useful liveness, but it must not
        # masquerade as acceptance-grade progress. Only a heartbeat carrying a
        # meaningful note uses the canonical ``heartbeat`` event kind.
        event_kind = "heartbeat" if note and str(note).strip() else "liveness"
        payload = (
            {"note": str(note).strip()}
            if event_kind == "heartbeat"
            else {"source": "runtime_activity"}
        )
...
def _resolve_worker_cli_toolsets(hermes_home: Optional[str]) -> list[str]:
    ...
    try:
        ...
        # Discovery makes plugin names known but never adds them implicitly.
        # The spawn allowlist remains the literal profile declaration, minus
        # the non-runtime ``no_mcp`` sentinel.
        discover_plugins()
        ...
        # Global registry aliases may have been populated by another profile
        # in this long-lived gateway, so validate only built-ins and MCP
        # servers enabled by this profile's own config.
        known_profile_toolsets = set(TOOLSETS) | set(known_mcp)
        unknown = [
            name for name in runtime_declared if name not in known_profile_toolsets
        ]
        if unknown:
            raise WorkerAuthorityResolutionError(
...
def _specialist_worker_toolsets(
    profile: str,
    configured: Optional[list[str]],
) -> Optional[list[str]]:
    """Project the model-visible toolsets for dispatcher specialist roles."""
    role = str(profile or "").strip().lower()
    if role in {"sol-verifier", "sol-builder"}:
        expected = {"file", "kanban", "specialist_test"}
        actual = set(configured or [])
        from toolsets import validate_toolset

        unknown = sorted(name for name in actual if not validate_toolset(name))
        if unknown:
            raise WorkerAuthorityResolutionError(
                "specialist worker toolset resolution failed closed: "
                "unknown toolset(s): " + ", ".join(unknown)
            )
        if actual != expected:
            raise WorkerAuthorityResolutionError(
                "specialist worker toolset resolution failed closed: expected "
                f"{sorted(expected)}, got {sorted(actual)}"
            )
        return sorted(expected)
    if role in {"sol-router", "sol-spec"}:
        return ["kanban"]
    return configured


def _specialist_spawn_env(
    base_env: dict[str, str],
    profile: str,
) -> dict[str, str]:
    """Strip credentials from specialist worker bootstrap environments."""
    role = str(profile or "").strip().lower()
    if role not in {"sol-verifier", "sol-builder", "sol-router", "sol-spec"}:
    ...
    safe = {key: value for key, value in sanitized.items() if key in allowed_exact}
    for key, value in base_env.items():
        if key.startswith("HERMES_KANBAN_") or key in {
            ...
            safe[key] = value
    safe["HERMES_WORKER_AUTHORITY"] = role
    return safe
...
def _default_spawn(
    ...
    *,
    ...
    # Specialist workers get a credential-stripped bootstrap environment and an
    # explicit authority role consumed by the final tool-dispatch guards.
    env = _specialist_spawn_env(env, profile_arg)

    # Resolve exact profile authority before log creation/rotation or Popen,
    # then apply the specialist-role projection. Any missing, broad, unknown,
    # or mismatched declaration fails closed before spawn.
    worker_toolsets = _specialist_worker_toolsets(
        profile_arg,
        _resolve_worker_cli_toolsets(env.get("HERMES_HOME")),
    )
    if not worker_toolsets:
        raise WorkerAuthorityResolutionError(
            f"worker CLI toolset resolution was empty for {profile_arg}"
        )
    ...
    if profile_arg not in {
        "sol-verifier",
        "sol-builder",
        "sol-router",
        "sol-spec",
    }:
        # Legacy workers retain profile-local hook behavior. Specialist roles
        # must not register arbitrary shell hooks because hooks bypass their
        # role/workspace authority boundary.
    ...
    if task.skills and profile_arg not in {
        "sol-verifier",
        "sol-builder",
        "sol-router",
        "sol-spec",
