...
def resolve_provider(
    ...
    *,
    ...
    if not _disable_paid_api_fallback and (
        has_usable_secret(_scoped_key_env("OPENAI_API_KEY"))
        or has_usable_secret(_scoped_key_env("OPENROUTER_API_KEY"))
    ):
        return "openrouter"
    ...
    *,
    ...
    Args:
        ...
        refresh_if_expiring: Refresh when the access token is within the skew
            window.
        refresh_skew_seconds: Number of seconds before expiry that should be
            treated as expiring. ``None`` (the default) derives the window per
            token via :func:`_xai_proactive_refresh_skew_seconds`, which caps
            at :data:`XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS` but shortens it
            for the ~15-minute device-code JWTs whose full-hour skew would
            force a refresh on every resolution and burn single-use refresh
            tokens into ``invalid_grant`` quarantine.
