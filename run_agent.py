...
class AIAgent:
    ...
    def _record_file_mutation_result(
        ...
        if not targets:
            return
        # Specialist file tools accept only one dispatcher-pinned workspace.
        # Canonicalize raw absolute/relative spellings before state updates so
        # a denied absolute attempt and later successful relative write to the
        # same expected file reconcile, while an unrelated escape remains a
        # distinct failure key.
        if os.environ.get("HERMES_WORKER_AUTHORITY"):
            ...
            if workspace:
                ...
                for target in targets:
                    ...
                    canonical.append(str(candidate.resolve(strict=False)))
