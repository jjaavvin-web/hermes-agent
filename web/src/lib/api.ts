...
export const api = {
  ...
  // fetchJSON now injects the session header on every /api/ call, but these
  // secret-revealing / OAuth-mutating routes keep the fork's explicit
  // getSessionToken() guard so the client fails CLOSED (throws) instead of
  // firing a tokenless request and relying on the server's 401.
