...
type OSView = "live" | "nexus" | "connectome" | "grid" | "git" | "projects";
...
function Hero({ snapshot, loading, onRefresh, onFocusSection }: HeroProps) {
  ...
  const infoDiagnostics = snapshot.info ?? [];
  const allDiagnostics = [...diagnostics, ...infoDiagnostics];
  const redCount = diagnostics.filter((d) => d.severity === "red").length;
  const amberCount = diagnostics.filter((d) => d.severity === "amber").length;
  ...
  const dirtyCount = repoSummary?.total_uncommitted ?? 0;
  const decisionCount = workCounts?.decisions ?? 0;
  const stalledCount = workCounts?.stalled ?? 0;
  const hasSecondaryMetrics = attention.length > 0 || dirtyCount > 0 || decisionCount > 0 || stalledCount > 0;
  ...
  return (
    <section className="flex-shrink-0 rounded-lg border bg-card p-4" style={{ borderColor: snapshot.overall === "green" ? undefined : overall.ring }} aria-label="Overall status">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="min-w-[180px] flex-1">
          <h2 className="font-mondwest text-display text-base tracking-[0.12em]" style={{ color: overall.color }}>
            {findingCount === 0 ? "All systems nominal" : `${findingCount} finding${findingCount === 1 ? "" : "s"} — ${redCount} red · ${amberCount} amber`}
          </h2>
      ...
      <div className="mt-3 flex flex-wrap gap-2.5 sm:gap-2" aria-label="OS primary hero chips">
        ...
        <MetricChip label="git readiness" value={`${repo?.readiness_pct ?? 0}%`} status={repoStatus} onClick={() => onFocusSection("repo")} />
        ...
        <button
          ...
          className="inline-flex min-h-[44px] max-sm:flex-shrink-0 items-center gap-1.5 rounded-full border border-border px-2.5 py-1 text-xs text-text-tertiary transition hover:bg-accent/30 hover:text-text-primary sm:min-h-0"
        >
      ...
      <button
        type="button"
        onClick={() => onFocusSection("repo")}
        className="mt-3 flex w-full items-center gap-2 rounded-lg border px-3 py-2 text-left transition hover:bg-accent/30"
        style={{ borderColor: STATUS_CFG[repoStatus].ring, background: STATUS_CFG[repoStatus].soft }}
        title="Focus repo section"
      >
        <StatusDot status={repoStatus} />
        <span className="flex-shrink-0 text-xs font-semibold uppercase tracking-[0.12em] text-text-tertiary">Best move</span>
        <span className="min-w-0 flex-1 break-words text-sm font-semibold text-text-primary">{bestMove || "No move available"}</span>
        <span className="flex-shrink-0 text-xs text-text-tertiary">focus repo →</span>
      </button>


      {moreOpen && (
        <div className="mt-3 rounded-lg border border-border bg-background/40 p-3" aria-label="OS secondary hero metrics">
          {hasSecondaryMetrics && (
            <div className="flex flex-wrap gap-2.5 sm:gap-2">
              ...
              {dirtyCount > 0 && <MetricChip label="dirty" value={dirtyCount} status="amber" onClick={() => onFocusSection("repo")} />}
              {decisionCount > 0 && <MetricChip label="decisions" value={decisionCount} status="amber" onClick={() => onFocusSection("work")} />}
              {stalledCount > 0 && <MetricChip label="stalled" value={stalledCount} status="amber" onClick={() => onFocusSection("work")} />}
            </div>
          )}
          {allDiagnostics.length > 0 && (
            <button
              type="button"
              onClick={() => setDiagDrawerOpen(true)}
              className="mt-2 text-xs text-text-tertiary underline decoration-dotted underline-offset-4 transition hover:text-text-primary"
            >
              Show all ({allDiagnostics.length}) diagnostics
            </button>
          )}
        </div>
      )}
      ...
      {diagnostics.length > 0 && (
        <ul className="mt-3 space-y-1.5" aria-label="Diagnostics preview">
          {diagnostics.slice(0, 4).map((diag, i) => <DiagnosticRow key={`${diag.source}-${i}`} diag={diag} />)}
        </ul>
      )}
      ...
      {allDiagnostics.length > 0 && (
        <button
          ...
          className="mt-2 text-xs text-text-tertiary underline decoration-dotted underline-offset-4 transition hover:text-text-primary"
        >
          Show all ({allDiagnostics.length}) diagnostics · info muted/excluded from counts
        </button>
      )}
      ...
      <DiagnosticsDrawer
        ...
        open={diagDrawerOpen}
        onOpenChange={setDiagDrawerOpen}
        redCount={redCount}
        amberCount={amberCount}
        infoCount={infoDiagnostics.length}
      />
    </section>
  );
}
...
export default function OSPage() {
  ...
  return (
    <div className={`bg-background p-4 text-text-primary ${view === "live" || view === "nexus" || view === "connectome" ? "flex min-h-[540px] flex-col lg:h-[calc(100dvh-112px)]" : "min-h-0"}`}>
      ...
      <div className="mb-3 flex flex-shrink-0 flex-wrap items-center gap-x-1.5 gap-y-2 text-xs text-text-tertiary">
        ...
        <div className="ml-auto flex flex-shrink-0 overflow-hidden rounded-md border border-border" role="group" aria-label="OS view">
          <button type="button" onClick={() => setView("live")} aria-pressed={view === "live"} className={`flex min-h-[44px] items-center gap-1.5 px-2.5 py-1 text-xs font-semibold transition sm:min-h-0 ${view === "live" ? "bg-accent/40 text-text-primary" : "text-text-tertiary hover:text-text-primary"}`}><LayoutDashboard className="h-3.5 w-3.5" />Live</button>
          <button type="button" onClick={() => setView("nexus")} aria-pressed={view === "nexus"} className={`flex min-h-[44px] items-center gap-1.5 border-l border-border px-2.5 py-1 text-xs font-semibold transition sm:min-h-0 ${view === "nexus" ? "bg-accent/40 text-text-primary" : "text-text-tertiary hover:text-text-primary"}`}><Workflow className="h-3.5 w-3.5" />Nexus</button>
      ...
      {view === "live" ? (
        <div className="mt-3 h-[70vh] min-h-[420px] min-w-0 overflow-hidden rounded-lg border border-border bg-black/20 lg:h-auto lg:flex-1">
          <iframe src="/nexus" title="Nexus v2" className="block h-full w-full border-0" />
        </div>
      ) : view === "nexus" ? (
        ...
        <div className="min-h-0"><OSProjects /></div>
      ) : (
        <>
