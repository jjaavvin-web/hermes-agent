...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        # Upstream v0.20 moved the /help body into the shared registry executor.
        # Fork: the originating platform must still reach it, because
        # ``gateway_help_lines(platform=...)`` hides the Discord-only ``/sol``
        # intake command from every other surface's help listing.
        platform = getattr(getattr(event, "source", None), "platform", None)
        reply = execute_command(
            "help",
            CommandContext(surface="gateway", options={"platform": platform}),
        )
    ...
    async def _handle_commands_command(self, event: MessageEvent) -> str:
        ...
        # Fork: forward the originating platform so the shared executor's
        # ``gateway_help_lines(platform=...)`` keeps hiding the Discord-only
        # ``/sol`` intake command from other surfaces (see _handle_help_command).
        platform = getattr(getattr(event, "source", None), "platform", None)
        ...
        reply = execute_command(
            ...
            CommandContext(
                ...
                options={"page_size": page_size, "platform": platform},
            ),
        )
        return _telegramize_command_mentions(
            reply.text,
