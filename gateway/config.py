    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if isinstance(gateway_section, dict):
                ...
                # Fork: bridge the nested gateway.max_concurrent_agent_runs knob
                # (inbound gateway/webhook run cap) through to gw_data["gateway"]
                # where resolve_max_concurrent_agent_runs() looks for it.
                if "max_concurrent_agent_runs" in gateway_section:
                    _nested_gw = gw_data.get("gateway")
                    if not isinstance(_nested_gw, dict):
                        _nested_gw = {}
                        gw_data["gateway"] = _nested_gw
                    _nested_gw["max_concurrent_agent_runs"] = gateway_section[
