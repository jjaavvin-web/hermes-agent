...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._wt_paths: Dict[str, str] = {}
        self._wt_route_init_failed: Dict[str, bool] = {}
        self._wt_path: Optional[str] = None   # default relay compatibility
    ...
    def _ensure_relay_worktree(
        self, route_name: str = "relay", route_config: Optional[dict] = None,
    ) -> Optional[str]:
        ...
        if route_name == "relay" and not route_config and self._wt_path is not None:
            if Path(self._wt_path).is_dir():
                return self._wt_path
        cached_path = self._wt_paths.get(route_name)
        if cached_path is not None and Path(cached_path).is_dir():
            return cached_path
        if self._wt_route_init_failed.get(route_name, False):
            return None
        try:
            from hermes_constants import get_hermes_home
            ...
            if route_name == "relay" and self._wt_init_failed:
                return None
            if wt_dir.is_dir():
            ...
            res = subprocess.run(
                add_cmd, capture_output=True, text=True, encoding="utf-8",
            ...
            if res.returncode != 0 or not wt_dir.is_dir():
                ...
                self._wt_route_init_failed[route_name] = True
                return None
            self._wt_paths[route_name] = str(wt_dir)
            if route_name == "relay":
                self._wt_path = str(wt_dir)
    ...
    async def _handle_webhook(self, request: "web.Request") -> "web.Response":
        ...
        if self._wt_enabled:
            if self._per_delivery_wt_enabled:
                ...
                # Explicit route overrides select a route worktree. Without
                # overrides, keep the established singleton rollback path.
                if not any(
                    route_config.get(key)
                    for key in ("worktree_branch", "worktree_base", "worktree_dir")
                ):
                    _wt_for_run = await asyncio.to_thread(self._ensure_relay_worktree)
                else:
                    _wt_for_run = await asyncio.to_thread(
                        self._ensure_relay_worktree, route_name, route_config,
                    )
