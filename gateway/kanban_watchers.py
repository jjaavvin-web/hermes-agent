...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_notifier_watcher(self, interval: float = 5.0) -> None:
        ...
        **Multi-board:** iterates every board discovered on disk per
        ...
        """
        # Upstream v0.20: dispatch and delivery have separate ownership. A
        # deployment may run one dispatcher while each profile has its own
        # gateway credentials; those adapter-owning gateways must still poll and
        # deliver their own subscriptions. Legacy rows without a
        # notifier_profile are visible only while this process holds the actual
        # singleton dispatcher lock. That per-profile ownership filter is
        # applied per-subscription below and is preserved here.
        # DISP-1 still fail-closes worker spawning in the dispatcher watcher.
        # Notification delivery is a separate capability: profile gateways
