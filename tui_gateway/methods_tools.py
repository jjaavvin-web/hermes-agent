...
def _(rid, params: dict) -> dict:
    if os.environ.get("HERMES_WORKER_AUTHORITY"):
        return _err(rid, 5011, "MCP reload denied for specialist worker")
...
def _(rid, params: dict) -> dict:
    ...
    if name == "learn":
        ...
        # Fork UX: surface a sys-line notice so the TUI shows what the
        # submitted turn is doing (fork-original; upstream sends bare).
        notice = (
        ...
        return _ok(
            rid,
            {"type": "send", "notice": notice, "message": build_learn_prompt(arg)},
        )
    if name == "init":
