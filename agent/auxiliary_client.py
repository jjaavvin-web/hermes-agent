...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        # v0.20 keeps that helper and adds the forward-progress hook below.
...
def _resolve_auto_route(
    ...
    v0.20 merge: upstream's multi-step discovery chain (main provider → task
    fallback chain → aggregator chain) stays disabled — it is exactly the paid
    fallback this fork closes.  Upstream's MoA *virtual-provider* resolution is
    kept, because it is a correctness guard rather than a routing policy: this
    function owns the IMPLICIT "main provider is moa" case (``resolve_provider_client``
    only unwraps an EXPLICIT ``provider: moa``), and an unresolved MoA facade
    would otherwise hand a preset NAME as a model id plus a ``moa://local``
    base_url and placeholder api_key into the sanctioned resolution.

    ``task`` is accepted for signature parity with upstream v0.20 callers; it is
    unused here because no task-scoped fallback chain is reachable under the
    fail-closed policy.
    """
    global auxiliary_is_nous
    auxiliary_is_nous = False

    # ── v0.20 MoA virtual-provider resolution (implicit case) ──────────────
    runtime = _normalize_main_runtime(main_runtime)
    ...
    if main_provider.strip().lower() == "moa":
        ...
        sanitized = dict(runtime)
        if agg_provider and agg_model:
            sanitized["provider"] = agg_provider
            sanitized["model"] = agg_model
        else:
            # Unresolvable preset (renamed/deleted/malformed aggregator slot):
            # fail closed rather than letting the facade leak downstream.
            sanitized.pop("provider", None)
            sanitized.pop("model", None)
        # The MoA facade's base_url ("moa://local") and placeholder api_key
        # belong to the virtual provider, not to any real endpoint — drop them
        # so the sanctioned route resolves through its own credentials.
        for facade_field in ("base_url", "api_key", "api_mode", "requested_provider"):
            sanitized.pop(facade_field, None)
        main_runtime = sanitized
    ...
    return client, resolved or _SANCTIONED_AUTO_MODEL, _SANCTIONED_AUTO_PROVIDER
    ...
    *,
    ...
    try:
        ...
        if should_fallback and (is_auto or is_capacity_error):
            ...
            # v0.20 merge: upstream re-enables a paid discovery chain here
            # (_try_configured_fallback_chain → _try_main_fallback_chain →
            # _try_payment_fallback) for the `auto` branch. That is precisely
            # the spend path this fork closes, so the fork's fail-closed gate is
            # kept instead — the two are mutually exclusive policies and the
            # restrictive one wins. The non-auto/non-sanctioned `else` branch
            # below still runs upstream's configured fallback chain unchanged.
    ...
    *,
    ...
    try:
        ...
        if should_fallback and (is_auto or is_capacity_error):
            ...
            # routes after a capacity error.
            fb_client, fb_model, fb_label = (None, None, "")
            # Same fail-closed billing gate as the sync path above; upstream's
            # v0.20 paid discovery chain stays disabled for `auto` / the
            # sanctioned aux route.
            fail_closed_aux = is_auto or _normalize_aux_provider(resolved_provider) == _SANCTIONED_AUTO_PROVIDER
            if fail_closed_aux:
                logger.error(
