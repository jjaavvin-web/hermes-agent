...
def init_agent(
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server", "claude_cli_subprocess"}:
    ...
    if agent.api_mode == "claude_cli_subprocess":
        # Subprocess/TTY runtime: no OpenAI/Anthropic SDK client is constructed.
        # The conversation loop forwards the whole turn to
        # agent.claude_cli_runtime, which launches interactive Claude Code on
        # the user's claude.ai OAuth/Max session.
        agent.api_key = ""
        agent.client = None
        agent._client_kwargs = {}
        if not agent.quiet_mode:
            print(f"🤖 AI Agent initialized with model: {agent.model} (Claude CLI subprocess)")
        ...
        # No-op for non-Mantle base URLs. Import + call are gated behind
        # agent.provider == "bedrock" so non-Bedrock sessions never trigger
        # the lazy_deps.ensure('provider.bedrock') side effect on import.
        if agent.provider == "bedrock":
            try:
                ...
                if "bedrock-mantle." in str(client_kwargs.get("base_url", "")):
                    raise
    ...
    # Specialist workers must never import configurable provider code.
    ...
    # Specialist workers must not import configured context-engine modules or
    # execute general plugin discovery during agent initialization.
    if os.environ.get("HERMES_WORKER_AUTHORITY"):
