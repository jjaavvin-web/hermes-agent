...
def _supported_compression_kwargs(
    ...
    *,
    ...
    ``preservation_context`` is the fork's CRITICAL PRESERVATION CONTEXT
    channel (commit 83f65db64). Upstream v0.20 independently added
    ``memory_context`` for the same provider payload; both kwargs are offered
    here and each is included only when the target engine declares it, so
    neither side's injection path is lost and older engines still work.
    """
    candidates = {
    ...
    if preservation_context:
        candidates["preservation_context"] = preservation_context
    try:
...
def compress_context(
    ...
    *,
    ...
    try:
        ...
        # and the fork's ``preservation_context`` carry the SAME
        # on_pre_compress() payload into two different summariser prompt
        # sections, so the hook is called once (above) and the sanitised value
        # feeds both kwargs. Sanitised — not the fork's raw string — because
        # sanitize_memory_context() is the egress redaction boundary and the
        # more restrictive option is the correct one here.
        #
        # The fork's old ``except TypeError`` double-call fallback is
        # deliberately gone: _supported_compression_kwargs() inspects the engine
        # signature up front, which covers the same strict-plugin case without
        # re-entering a stateful compressor.
        preservation_context = memory_context
        ...
        if (
            ...
            and "memory_context" not in compress_kwargs
            and "preservation_context" not in compress_kwargs
        ):
            engine_name = getattr(
                agent.context_compressor,
                "name",
