...
KANBAN_GUIDANCE = (
    ...
    "`kanban_comment` first. Then: if you worked in a **git worktree** (a "
    "code card whose workspace is a worktree) and committed your change "
    "there, run the terminal command `hermes kanban open-pr <your-task-id>` "
    "— it pushes your branch, opens a needs-human PR, records the url, and "
    "blocks the card for review in one step (no separate `kanban_block` "
    "needed). Otherwise end with "
    "`kanban_block(reason=\"review-required: <one-line summary>\")` so a "
    "reviewer can approve+unblock or request changes. The merge stays human "
    "either way — reviewing-then-completing is more honest than auto-"
    "completing work that still needs eyes on it.\n"
...
def build_codex_session_prompt(worktree_path: Optional[str]) -> str:
    ...
    return (
        ...
        "## Workflow (conversational — no rigid phase gates)\n\n"
        "1. **Read the ISA at the path above FIRST** for its Goals / Constraints / ISCs. "
        "It's your plan + scratchpad; keep the ISC checkboxes honest as you go.\n"
        "2. **Work in your worktree** (your cwd, above) — code changes belong there. The "
        "sandbox only refuses writes into the live source checkout or another thread's "
        "worktree; shared config (e.g. `~/.hermes/config.yaml`), dotfiles and other "
        "non-repo paths are fine to edit when the task genuinely needs it.\n"
        "3. **Implement against the ISCs.** Commit as you finish iterations: "
        "`git -C " + worktree_path + " add -A && git -C " + worktree_path +
        " commit -m \"<short msg>\"` from a Bash tool call. Tick the matching ISC `[x]`.\n"
        "4. **When implementation is complete, run `/review` in this thread** to trigger "
        "independent peer review. **Do not push or open a PR manually** — the merge broker "
        "takes over on `APPROVE`. On `REVISE`, address the feedback and run `/review` again.\n"
        "5. The dispatcher marks the session complete and archives the thread once the PR merges.\n\n"
