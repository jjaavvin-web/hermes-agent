...
KANBAN_GUIDANCE = (
    ...
    "this same task needs review before it is final: if you worked in a "
    "**git worktree** (a code card whose workspace is a worktree) and "
    "committed your change there, run the terminal command "
    "`hermes kanban open-pr <your-task-id>` — it pushes your branch, opens a "
    "needs-human PR, records the url, and blocks the card for review in one "
    "step (no separate `kanban_request_review` call needed). For any other "
    "task, call `kanban_request_review(summary=..., metadata=..., "
    ...
    "external escalation. Neither path trips unblock-loop detection.\n"
