...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    return {
        ...
        for p in [
            ...
            # poison/truncate it (plaintext-equivalent to .env). Retained
            # alongside the v0.20 encrypted cache because legacy plaintext
            # caches still exist on disk for pre-migration profiles.
            str(hermes_home / "cache" / "bws_cache.json"),
            str(hermes_root / "cache" / "bws_cache.json"),
            # Bitwarden Secrets Manager encrypted disk cache (upstream v0.20).
...
# Fork keeps the cross-profile write guard (upstream retired it); see DIVERGENCES.md V2.
