...
def _resolve_codex_thread_cwd(agent) -> tuple[str, str]:
    ...
    # v0.20 re-anchor: upstream moved the unbound fallback off raw os.getcwd()
    # onto agent.runtime_cwd.resolve_agent_cwd(), which honours the _SESSION_CWD
    # contextvar and TERMINAL_CWD before falling back to the process cwd. Only
    # this *legacy, fully-unbound* leg changes — every confinement branch above
    # still fails closed first.
    try:
        from agent.runtime_cwd import resolve_agent_cwd

        return str(resolve_agent_cwd()), "legacy_process_cwd"
    except Exception:
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Supersedes the narrower item/started-only bridge from #38835 that the
        # fork carried locally (_codex_note_to_tool_progress) — the v0.20 bridge
        # is a strict superset of it (same item types, plus tool.completed,
        # stream/reasoning deltas and interim assistant messages).
        #
        # Constructed through the fork's module-level ``session_cls`` seam
        # (not the direct ``CodexAppServerSession`` symbol) so tests can
        # monkeypatch the constructor without importing the transport at
        # module import time.
