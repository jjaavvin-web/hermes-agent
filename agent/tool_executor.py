    ...
    *,
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        ...
        if (
            ...
            and os.environ.get("HERMES_WORKER_AUTHORITY")
        ):
            block_error_type = "specialist_authority_block"
            try:
                from hermes_cli.worker_authority import authorize_current_worker

                authority = authorize_current_worker(function_name, final_args)
                if not authority.allowed:
                    block_message = authority.reason
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    # Do not mutate activity state at batch setup: authority is evaluated on
    # final post-middleware arguments inside each worker. Authorized calls
    # touch activity in _begin_tool_execution; a fully denied batch therefore
    # has no checkpoint/progress/activity side effects.
