...
``hermes unpause`` removes it. While the sentinel exists:
...
def disengage() -> bool:
    ...
    ``hermes unpause`` from a profile gateway still clears an operator pause
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            ...
            "run `hermes unpause` to pick things back up."
        )
    return (
