...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    # Agent-loop tools (todo, memory, delegation, context-engine providers)
    # bypass model_tools/registry dispatch. Enforce specialist authority on the
    # final post-middleware payload before any direct handler can run.
    if os.environ.get("HERMES_WORKER_AUTHORITY"):
        try:
            ...
            return json.dumps(
                {"error": f"specialist authority guard failed closed: {authority_err}"},
                ensure_ascii=False,
            )
