...
class CredentialPool:
    ...
    def has_available(self) -> bool:
        ...
        #
        # Fork: the probe must answer the SAME question selection will ask, so
        # it honours the active auth-type restriction. An OAuth-only pool with
        # nothing but API-key entries left is NOT "available".
        with self._lock:
    ...
    def _available_entries(
        ...
        *auth_type* is supplied (fork: OAuth-only runtime guard), only entries
        of that auth type are considered for selection while preserving the
        underlying pool order and statuses.
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            # Fork: an OAuth-only runtime must never see API-key entries.
    ...
    def _select_unlocked(
        ...
        *auth_type* (fork) confines selection to one auth type so an
        OAuth-only runtime can never rotate onto an API-key entry.
        """
        available, pending_refresh = self._available_entries(
            clear_expired=True, refresh=refresh, auth_type=auth_type,
        )
        if not available:
            self._current_id = None
    ...
    def select_anthropic_oauth_only(self) -> PooledCredential:
        ...
        with self._lock:
            ...
            # pending_refresh is only ever populated for openai-codex /
            # xai-oauth; this branch is anthropic-only, so there is nothing to
            # run outside the lock here.
            selected, _pending = self._select_unlocked(auth_type=AUTH_TYPE_OAUTH)
    ...
    def peek(self) -> Optional[PooledCredential]:
        ...
        with self._lock:
            current = self._current_unlocked()
            # Fork: never surface a credential that violates the active
            # auth-type restriction (OAuth-only runtimes must not peek an
            # API-key entry); fall through to a restricted lookup instead.
            if current is not None and (
                self._selection_auth_type is None
                or current.auth_type == self._selection_auth_type
            ):
                return current
            available, _pending = self._available_entries(
                auth_type=self._selection_auth_type
        ...
        *,
        ...
        with self._lock:
            ...
            if credential_id:
                entry = next(
                    (
                        ...
                        # Fork: an explicit id must still respect the active
                        # auth-type restriction.
                        and (selection_auth_type is None or e.auth_type == selection_auth_type)
                    ),
                    None,
                )
            ...
            if entry is None and api_key_hint:
                ...
                # Fork: never cross an active auth-type restriction (for example
                # OAuth-only pools must not rotate into API-key credentials).
                entry = next(
                    (
                        e for e in self._entries
                        if e.runtime_api_key == api_key_hint
            ...
            if entry is None and identity_supplied:
                ...
                next_entry, _pending = self._select_unlocked(
                    refresh=False, auth_type=selection_auth_type,
                )
                avail, _ = self._available_entries(auth_type=selection_auth_type)
            ...
            self._unmatched_rotation_streak = 0
            if entry is None:
                # ``_current_unlocked`` (not ``current()``) — we already hold
                # the non-reentrant lock here.
                entry = self._current_unlocked()
                # Fork: a current entry outside the active auth-type
                # restriction must not be marked/rotated on its behalf.
                if entry is not None and (
                    selection_auth_type is not None and entry.auth_type != selection_auth_type
                ):
                    entry = None
                entry = entry or self._select_unlocked(
                    refresh=False, auth_type=selection_auth_type,
                )[0]
            if entry is None:
                return None
    ...
    def _acquire_lease_under_lock(
        ...
        with self._lock:
            # Fork hardening: NO unconditional credential_id short-circuit
            # here.  Upstream leased a requested id straight through, which
            # skips the auth-type / exhausted / DEAD / refresh guards and lets
            # a caller lease a credential normal selection would refuse.  The
            # requested id is validated against ``available`` below instead.
            available, pending_refresh = self._available_entries(
                clear_expired=True,
                refresh=True,
                auth_type=self._selection_auth_type,
            ...
            if entry is None:
                if api_key_hint:
                    ...
                    entry = self._current_unlocked() or self._select_unlocked(
                        refresh=False, auth_type=self._selection_auth_type,
                    )[0]
            if entry is None:
                return None
            self._current_id = entry.id
