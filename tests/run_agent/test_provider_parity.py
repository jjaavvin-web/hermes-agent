...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_openrouter_not_used_by_auto(self, monkeypatch):
        ...
        # MERGE DIVERGENCE (v0.20): upstream re-asserts the aggregator chain here
        # (model == _OPENROUTER_MODEL, base_url containing "openrouter"). The fork
        # deliberately inverts that assertion: burn edc7e0133 (no-paid-fallback
        # doctrine) makes a literal `provider: auto` fail-closed, so a present
        # OPENROUTER_API_KEY must NOT be spent by the auto route. Fail-closed wins.
    ...
    def test_nous_not_used_by_auto(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_nous_auth", return_value=nous_auth), \
            ...
            client, model = get_text_auxiliary_client()
        # MERGE DIVERGENCE (v0.20): upstream asserts model == _NOUS_MODEL here.
        # Same inversion as the OpenRouter case above — 'auto' resolves only to the
        # sanctioned subscription provider, never to a discovered aggregator.
        assert client is None
        assert model is None
        mock.assert_not_called()
