"""Legacy command_allowlist shapes must not grant per-character approvals."""
import yaml
...
def _write_and_reload(tmp_path, value):
    (tmp_path / "config.yaml").write_text(
        yaml.safe_dump({"command_allowlist": value}),
        encoding="utf-8",
    )
...
def test_legacy_string_allowlist_recovers_only_string_lists(tmp_path, monkeypatch, caplog):
    ...
    for value in ([description], yaml.safe_dump([description])):
        _write_and_reload(tmp_path, value)
