...
def test_nous_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    available, pending_refresh = pool._available_entries(clear_expired=True)
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    available_before, pending_before = pool._available_entries(
        clear_expired=True,
        refresh=False,
    )
    ...
    available, pending_refresh = pool._available_entries(
        clear_expired=True,
        refresh=False,
    )
    assert pending_refresh == []
...
def test_xai_oauth_terminal_refresh_clears_auth_json_and_removes_pool_entries(
    ...
    # (pool is now empty of device-code OAuth entries and current is None).
    assert pool.try_refresh_current() is None
    assert refresh_calls["count"] == 1
