...
class TestCallLlmPaymentFallback:
    ...
    @pytest.mark.skip(
        reason="FORK DOCTRINE (v2026.7.1 merge): upstream #21165 expects a 401 on "
        "provider=auto to fall back to a paid OpenRouter client at request time. "
        "The fork's spend rail (edc7e0133) deliberately fail-closes auto instead — "
        "_try_payment_fallback is intentionally dead code (zero call sites) so an "
        "expired-credential turn can never silently bill a paid API. Fork behavior "
        "is pinned by the union tests 'Expired Codex → auto fail-closes; never "
        "silent paid fallback' in this file."
    )
...
class TestAuxiliaryFallbackLayering:
    ...
    @pytest.mark.skip(
        reason="FORK DOCTRINE (v2026.7.1 merge): upstream #52228's fixture picks "
        "provider 'openai-codex' — exactly the fork's sanctioned burn route, which "
        "fail_closed_aux fail-closes before any fallback layer is consulted. "
        "Upstream's actual feature SURVIVES the merge: rate-limit is in "
        "is_capacity_error and explicit NON-sanctioned providers do fall back on "
        "429; only this fixture's provider choice collides with the fork rail."
    )
