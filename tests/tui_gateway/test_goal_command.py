...
def server(hermes_home, monkeypatch):
    ...
    #
    # MERGE (v0.20): the fork pinned the same four attributes with a bare
    # ``setattr``; upstream's ``monkeypatch.setattr`` does the same pinning
    # AND restores the originals on teardown, so it supersedes the fork
    ...
    # Fork-side addition, kept: clear inherited session state on the way IN
    # too. The module is a process-wide singleton, so a sibling test file
    # that imported it earlier can leave sessions behind; post-clear alone
    # only protects whoever runs after us.
    mod._sessions.clear()
    mod._pending.clear()
    mod._answers.clear()
    yield mod
