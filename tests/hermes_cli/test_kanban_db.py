...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    with _kb.connect() as conn:
        host = _kb._claimer_id().split(":", 1)[0]
        tid = _kb.create_task(conn, title="rl", assignee="a")
        ...
        for i in range(6):
            ...
            _kb.claim_task(conn, tid, claimer=f"{host}:w{i}")
            conn.execute(
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    from unittest.mock import patch as _patch
    import hermes_cli.kanban_db as _kb
    _state = importlib.import_module("hermes_state")
    ...
    _state._wal_fallback_warned_paths.clear()
    # Assume a fixed SQLite so the WAL-reset gate doesn't short-circuit.
    monkeypatch.setattr(
        _state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    t = _kb.create_task(conn, title="post-silent-fallback task")
