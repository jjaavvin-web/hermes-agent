...
def _make_hermes_tree(root: Path) -> None:
    ...
    # Fork (PR#70): plant real secret-shaped files so the exclusion tests have
    # something that MUST never land in a backup/snapshot.
    ...
    # Upstream v0.20: these must be REAL sqlite databases — _safe_copy_db uses
    # the sqlite backup API, which rejects the fork's old fake byte blobs.
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        Fork/PR#70 invariant: secret-bearing files must never enter the zip.
        Upstream v0.20 dropped this test; the fork keeps it.
        """
