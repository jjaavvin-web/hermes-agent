...
def test_codex_apply_patch_callback_accept_must_route_through_check_all_command_guards(
    ...
    assert session._decide_apply_patch_approval({"reason": "apply test patch"}) == "decline"
    # Guard is called with the FIXED "apply_patch" label, never the
    # attacker-controllable reason string (hardened post-f00fbbef6^; see
    # KEYSTONE #1b comment in codex_app_server_session._decide_apply_patch_approval).
    assert observed == ["apply_patch"]
    assert recorder.calls == []
