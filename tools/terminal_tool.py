...
def _get_env_config() -> Dict[str, Any]:
    ...
    # v0.20: a remote ``~`` for an ssh backend must stay unexpanded (expanding
    # it here would bake in the LOCAL home path).
...
def _resolve_command_cwd(
    *,
    ...
    Under required worktree confinement this delegates to the shared
    fail-closed resolver (fork rail t_0113eacc F5c) and a bound worktree
    outranks everything, so a lane shell can never step outside its
    isolated checkout. Outside confinement, explicit ``workdir=`` overrides
    everything.
    ...
    Every resolved value is handed to ``record_runtime_execution_cwd`` so the
    finalize-time adoption audit compares the cwd actually used (F4).
    ...
    # v0.20 replaced the fork's ``env.cwd`` read with the per-session cwd
    # RECORD (``env`` is no longer a parameter): same "live session cwd"
    # intent, without the shared-environment cross-talk. The fork's F4
    # runtime-cwd recording still wraps whatever that resolves to — on the
    # container-unusable fallback path too, so the adoption audit always sees
    # the cwd actually used.
    ...
    if (
        ...
        return default_cwd
    resolved_cwd = recorded or default_cwd
    record_runtime_execution_cwd(resolved_cwd)
    return resolved_cwd
...
def terminal_tool(
    ...
    try:
        ...
        if os.environ.get("_HERMES_GATEWAY") == "1":
            ...
            # Same fail-closed handling as the two execution call sites: under
            # required worktree confinement this resolver RAISES rather than
            # falling back to the live tree, and the v0.20 lifecycle guard is a
            # new call site that must surface that as a blocked result instead
