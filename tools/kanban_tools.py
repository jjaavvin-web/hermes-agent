...
def _enforce_specialist_handler_authority(
    tool_name: str,
    args: dict,
) -> Optional[str]:
    """Keep direct registry/internal lifecycle calls inside authority."""
    if not os.environ.get("HERMES_WORKER_AUTHORITY"):
        return None
    try:
        from hermes_cli.worker_authority import authorize_current_worker

        decision = authorize_current_worker(tool_name, args)
    except Exception as exc:
        return tool_error(
            f"specialist authority guard failed closed: {exc}"
        )
    if decision.allowed:
        return None
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    authority = str(
        os.environ.get("HERMES_WORKER_AUTHORITY") or ""
    ).strip().lower()
    if authority:
        profile = str(os.environ.get("HERMES_PROFILE") or "").strip().lower()
    ...
    try:
        ...
        try:
            try:
                active_run = kb.heartbeat_worker(
                    conn,
                    tid,
                    note=None,
...
def _handle_complete(args: dict, **kw) -> str:
    ...
    authority_err = _enforce_specialist_handler_authority(
        "kanban_complete",
        args,
    )
    if authority_err:
