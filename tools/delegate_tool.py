...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "notify_on_complete=True) instead. delegate_task normally runs "
        "synchronously, but supports background=True for detached runs: it "
        "returns a delegation handle immediately, the parent turn continues, "
        "and the result re-enters the conversation as a new turn when the "
        "child finishes. A detached background child's only hard wall-clock "
        "bound is delegation.child_timeout_seconds (600 seconds in this "
        "deployment); inactivity_timeout governs the already-ended parent "
        "turn, not the child, and the async watcher fires only on completion.\n\n"
        "IMPORTANT:\n"
