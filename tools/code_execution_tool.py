...
def _resolve_child_cwd(mode: str, staging_dir: str, task_id: str = "") -> str:
    ...
    Fork (F4, t_0113eacc): EVERY rung of that ladder — including the strict
    staging tmpdir — is routed through the shared ``resolve_confined_cwd``
    resolver, so when worktree confinement is required this fails closed to
    the active valid worktree and never falls back to a recorded session
    cwd, a registered override, TERMINAL_CWD, ``os.getcwd()``, or staging.
    """
    if mode != "project":
        return resolve_confined_cwd(staging_dir)
    if task_id:
        ...
        if recorded and os.path.isdir(recorded):
            return resolve_confined_cwd(recorded)
        ...
        if session_cwd and os.path.isdir(session_cwd):
            return resolve_confined_cwd(session_cwd)
