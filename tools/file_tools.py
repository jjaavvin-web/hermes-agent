...
def _resolve_path_for_task(filepath: str, task_id: str = "default") -> Path | PurePosixPath:
    ...
    Fork rails layered on top of that base resolution, for RELATIVE host
    paths only (both retained across the v0.20 merge):
      * Codex-parallel-workflow per-thread worktree (P1.2): when the current
        async task is inside a tracked codex thread with a live worktree
        binding, the relative path resolves under that worktree — it wins
        over :func:`_resolve_base_dir` (live terminal cwd / registered
        override / ``TERMINAL_CWD`` / ``os.getcwd()``). This closes the gap
        where the first Edit/Write of a turn, before any ``cd`` updated live
        tracking, would land in the live tree instead of the worktree.
      * Worktree confinement (t_0113eacc F3): when the turn REQUIRES an
        active worktree and none is bound (or the bound one is gone), the
        relative path is denied outright rather than falling back to the
        live terminal cwd, ``TERMINAL_CWD``, or the process cwd.
    """
    container_paths = _uses_container_paths(task_id)
    if container_paths:
    ...
    if p.is_absolute():
        return p.resolve()
    # Fork worktree-confinement rail (t_0113eacc F3): when a codex turn
    # requires an active worktree, a relative path must NEVER fall back to
    # the live terminal cwd / TERMINAL_CWD / process cwd — fail closed.
    ...
    if codex_wt:
        return (Path(codex_wt) / p).resolve()
    # v0.20: base resolution moved into _resolve_base_dir (live cwd record ->
    # registered override -> TERMINAL_CWD -> process cwd, MSYS-translated).
...
def _specialist_atomic_workspace_write(path: str, content: str) -> str:
    """Write relative to a stable workspace fd without following symlinks."""
    ...
    if (
        ...
        return tool_error(
            "specialist authority denied invalid workspace-relative path"
        )
    ...
    try:
        for component in raw.parts[:-1]:
            try:
                ...
                os.mkdir(component, mode=0o755, dir_fd=dir_fd)
                next_fd = os.open(component, flags_dir, dir_fd=dir_fd)
            os.close(dir_fd)
            dir_fd = next_fd

        leaf = raw.parts[-1]
        # Never truncate an existing inode: it may be a hardlink to a file
        # outside the workspace. Write a fresh inode and atomically replace
        # the directory entry instead.
        temp_leaf = f".{leaf}.hermes-{uuid.uuid4().hex}.tmp"
        file_flags = (
            os.O_WRONLY
            | os.O_CREAT
            | os.O_EXCL
        ...
        file_fd = os.open(temp_leaf, file_flags, 0o644, dir_fd=dir_fd)
        try:
        ...
        return tool_error(f"specialist atomic write denied: {exc}")
    finally:
        os.close(dir_fd)

    resolved = str((Path(workspace) / raw).resolve(strict=True))
    return json.dumps(
        {
            "success": True,
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    if os.environ.get("HERMES_WORKER_AUTHORITY") == "sol-builder":
        return _specialist_atomic_workspace_write(path, content)
    try:
        ...
        # subagents can't interleave on the same file. Different paths remain
        # fully parallel. Specialist builders additionally revalidate the
        # canonical target immediately before opening it, closing symlink-swap
        # TOCTOU for any path that reaches this legacy branch.
        with file_state.lock_path(_resolved):
            if os.environ.get("HERMES_WORKER_AUTHORITY") == "sol-builder":
                try:
                    ...
                    current = str(_resolve_path_for_task(path, task_id))
                    if not decision.allowed or current != _resolved:
                        return tool_error(
                            "specialist authority denied: target changed after authorization"
                        )
                except Exception as authority_err:
                    return tool_error(
                        "specialist authority check failed closed: "
                        f"{authority_err}"
                    )
