...
HARDLINE_PATTERNS = [
    ...
    # Re-expressed 2026-07-04 (v2026.7.1 merge) with the upstream combinators
    # so the fork rule inherits the quoting/brace-bypass closures.
    (_RM_FLAG_PREFIX + _hardline_rm_path(
        r'(?:~|\$\{?HOME\}?|/home/[^/\s"\']+)/\.hermes(?:/|/\*)?|'
        r'(?:~|\$\{?HOME\}?|/home/[^/\s"\']+)/\.local/share/hermes-agent(?:/|/\*)?'
    ), "recursive delete of the Hermes state dir or agent install"),
...
def detect_hardline_command(command: str) -> tuple:
    ...
    """
    # v0.20 structural fail-closed guards first: a command we cannot parse
    # reliably (parser limit) or a malformed exec-bearing grep is blocked
    # before any pattern matching is trusted.
    ...
    # Fork credential-exfiltration pre-check (SEC-1) runs BEFORE the pattern
    # floor: an exfil hit at hardline severity blocks regardless of rm/root
    # shapes.
...
def detect_dangerous_command(command: str) -> tuple:
    ...
    """
    # v0.20 fail-closed structural guard first: an unparseably long command is
    # dangerous by construction.
    if _command_parser_limit_exceeded(command):
        return (True, _PARSER_LIMIT_DESCRIPTION, _PARSER_LIMIT_DESCRIPTION)

    # Fork credential-exfiltration pre-check runs BEFORE the pattern floor
    # (dangerous-severity leg of the two-step credential-taint rail), and
    # deliberately before v0.20's cleanup allow-list below so no allow path
    # can shadow the SEC-1 rail.
    exfil, severity, exfil_desc = _detect_credential_exfiltration(command)
    if exfil and severity == "dangerous":
        return (True, exfil_desc, exfil_desc)

    # v0.20 narrow allow-list: `rm -f <tmpdir>/hermes-verify-*` self-cleanup.
...
def _run_approval_gate(
    *,
    ...
    if is_gateway or env_var_enabled("HERMES_EXEC_ASK"):
        ...
        if notify_cb is not None:
            ...
            if choice == "session":
                ...
                # Fork rail t_ec1d82e1 / R5a, RE-ANCHORED onto v0.20's shared
                # gate: "always" on a destructive class (see
                # DESTRUCTIVE_APPROVAL_PATTERN_KEYS) must NOT be written to the
                # permanent command_allowlist -- it is downgraded to
                # session-only. v0.20 moved check_dangerous_command's tail into
                # this helper with the raw approve_permanent() sequence, which
    ...
    if choice == "session":
        ...
        # Fork rail t_ec1d82e1 / R5a, RE-ANCHORED (see the gateway branch
        # above): destructive-class "always" grants are session-only and are
        # never written to the permanent command_allowlist.
        downgrade_notice = _apply_always_choice(session_key, pattern_key)
            ...
            if not smart_denied_for_owner:
                for key, _, is_tirith in warnings:
                    if transport_choice == "session" or (
                        ...
                        # Fork rail (row 40): 'always' must route through _apply_always_choice so destructive-class keys stay session-only.
            ...
            if not smart_denied_for_owner:
                for key, _, is_tirith in warnings:
                    if choice == "session" or (choice == "always" and is_tirith):
                        ...
                        # Fork rail t_ec1d82e1: destructive classes are granted
                        # for the session only and never written to the
                        # permanent command_allowlist.
                        notice = _apply_always_choice(session_key, key)
                        if notice:
                            downgrade_notices.append(notice)
