...
def get_env_value(name: str, default=None):
    ...
    long-lived gateway processes do not keep a stale helper after
    ``hermes_cli.config`` is patched or reloaded.
    """
    try:
        from hermes_cli.config import get_env_value as _hermes_get_env_value
    except Exception:
        # Fork: broad (not ImportError-only) so a partially-initialised or
        # freshly-reloaded ``hermes_cli.config`` degrades to the raw
        # environment instead of raising out of an unrelated caller.
        return os.environ.get(name, default)

    # Upstream v0.20 (062d44bba) fail-closed: honor ``get_secret``'s verdict.
    # Deliberately NOT wrapped in try/except and deliberately NOT falling back
    # to ``os.environ`` on a miss -- a scope miss under a multiplexed gateway
    # turn must resolve to ``default`` and ``UnscopedSecretError`` must
    # propagate, rather than leaking another profile's raw environ value.
