...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # NOTE: self._nonconversational_messages is already initialised above
        # (see the 2026-06-22 "stranded after a return" fix). Upstream v0.20 also
        # assigns it in this block — deliberately NOT duplicated here.
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            async def on_message(message: DiscordMessage):
                # v0.20 refactor: Discord ingress policy + dispatch now live in
                # _dispatch_discord_message(). The fork's Codex P1/P1.5 thread hook
                # is re-anchored there, between admission and _handle_message, so it
                # still runs only for admitted messages and in the same async task.
            ...
            async def on_thread_create(thread):
                """Codex P1 hook — allocate a session for new threads — then
                normalize the event for gateway_platform_event subscribers.
                """
                if adapter_self._codex_dispatcher is not None:
                    try:
            ...
            async def on_thread_update(before, after):
                """Codex P1 hook — release a session on thread archive — then
                normalize the event for gateway_platform_event subscribers.
                """
                # Only fire the Codex archive hook on the archived transition
                # (False -> True); the generic normalize call below still runs
                # for every update (e.g. renames).
                if adapter_self._codex_dispatcher is not None and (
                    getattr(after, "archived", False) and not getattr(before, "archived", False)
                ):
                    try:
                        from gateway.codex_session_dispatcher import ThreadEvent  # noqa: PLC0415
    ...
    async def _dispatch_discord_message(self, message: Any) -> bool:
        ...
        if self._codex_dispatcher is not None:
            try:
                _ch_id = str(message.channel.id)
                if self._codex_dispatcher.is_tracked(_ch_id):
                    from gateway.codex_session_dispatcher import ThreadEvent  # noqa: PLC0415
                    from agent.codex_session_context import set_active_worktree  # noqa: PLC0415
                    await self._codex_dispatcher.on_thread_message(
                        ThreadEvent(
                            thread_id=_ch_id,
                            channel_id=_ch_id,
                    ...
                    _row = self._codex_dispatcher._load_state().get(
                        "sessions", {}
                    ).get(_ch_id)
