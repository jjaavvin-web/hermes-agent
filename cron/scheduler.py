    ...
    *,
    ...
    try:
        ...
        agent = AIAgent(
            ...
            skip_memory=True,  # Cron system prompts would corrupt user representations (fork policy; upstream ef04d846e9 deliberately not adopted)
...
def tick(
    ...
    *,
    ...
    try:
        ...
        # Recurring-fire ledger recovery: rows claimed by a tick that died
        # before mark_recurring_fire_complete are surfaced (and the ledger
        # compacted) at the top of every dispatching tick.
        reconcile_incomplete_recurring_fires()
        ...
        # Recurring-fire ledger: claim every due fire BEFORE next_run_at moves,
        # so a tick that dies mid-dispatch leaves an incomplete row that the
        # next tick's reconcile pass can see (fork invariant).
        ...
        def _process_job(job: dict) -> bool:
            ...
            if not claimed:
                ...
                mark_recurring_fire_complete(fire_ids.get(job["id"]), job["id"], success=False)
                return True
