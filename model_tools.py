...
def handle_function_call(
    ...
    # Dispatcher-spawned specialist workers carry an explicit authority role
    # and workspace. Enforce that policy at the final registry-dispatch seam so
    # aliases, Tool Search, and stale/global schemas cannot bypass authority.
    try:
        from hermes_cli.worker_authority import authorize_current_worker

        authority = authorize_current_worker(function_name, function_args)
        if not authority.allowed:
            return json.dumps({"error": authority.reason}, ensure_ascii=False)
    except Exception as authority_err:
        if os.environ.get("HERMES_WORKER_AUTHORITY"):
            return json.dumps(
                {
                    "error": (
                        "specialist authority guard failed closed: "
                        f"{authority_err}"
                    )
                },
                ensure_ascii=False,
    ...
    # Middleware may replace the effective payload. Re-authorize the final
    # arguments immediately before any hook or registry dispatch.
    if os.environ.get("HERMES_WORKER_AUTHORITY"):
        try:
            from hermes_cli.worker_authority import authorize_current_worker

            final_authority = authorize_current_worker(function_name, function_args)
            if not final_authority.allowed:
                return json.dumps(
                    {"error": final_authority.reason}, ensure_ascii=False
                )
        except Exception as authority_err:
            return json.dumps(
                {
    ...
    try:
        ...
        # Fork rail: re-authorize after the pre_tool_call 'modify' directive may have rewritten args (worker authority must see the final args).
        if os.environ.get("HERMES_WORKER_AUTHORITY"):
            try:
                from hermes_cli.worker_authority import authorize_current_worker

                post_hook_authority = authorize_current_worker(function_name, function_args)
                if not post_hook_authority.allowed:
                    return json.dumps(
                        {"error": post_hook_authority.reason}, ensure_ascii=False
                    )
            except Exception as authority_err:
                return json.dumps(
                    {
