...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            def _connect_and_init():
                ...
                self._conn.row_factory = sqlite3.Row
                # Enable incremental reclamation before schema creation for new
                # databases. Existing databases need an operator-run VACUUM;
                # never rewrite them merely because a connection is opened.
    ...
    def count_zombie_sessions_to_reap(self, grace_days: int = 7, inactive_days: int = 7) -> int:
        ...
        with self._read_ctx() as conn:
            return self._zombie_reap_candidate_count(conn, grace_days, inactive_days)
    ...
    # NOTE: _fts_table_exists now lives on SessionSearchMixin
    # (hermes_state_search.py) — upstream moved it there and widened the
    # except to sqlite3.DatabaseError. SessionDB inherits it.
    ...
    def run_db_maintenance(self) -> List[str]:
        ...
        """
        # Follow the lineage concurrent-writer rule: do not reset the WAL
        # from this transient maintenance process.
        steps = [
            "PRAGMA wal_checkpoint(PASSIVE)",
            "PRAGMA optimize",
        ...
        with self._lock:
            # These PRAGMAs mutate database state and belong on the writer
            # connection. Keep their operations explicit, like VACUUM above.
            self._conn.execute("PRAGMA wal_checkpoint(PASSIVE)")
    ...
    def maybe_auto_prune_and_vacuum(
        ...
        try:
            ...
            # Two gates, both required (merge of fork GWR-1/ARCH-9 + upstream
            # rate limit):
            #
            # 1. Rate limit (upstream): never VACUUM more often than once every
            #    min_vacuum_interval_days -- a large prune (e.g. the first one to
            #    cross retention_days on a DB with tens of thousands of rows) can
            #    free enough pages that pruned > 0 fires on every subsequent
            #    startup even though a VACUUM already ran recently. VACUUM on this
            #    DB's size (FTS5 shadow tables) is not cheap -- it holds an
            #    exclusive lock for the full rewrite.
            # 2. Trigger (fork): decouple compaction from prune-count. All
            #    sessions are usually inside retention so prune returns 0, which
            #    previously meant optimize_fts/VACUUM never fired. Fire on actual
            #    growth/fragmentation independent of how many rows the prune
            #    freed; a prune that freed rows still vacuums as before.
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.command == "reap-zombies":
        db = SessionDB(db_path=args.db_path, read_only=args.dry_run)
        try:
            if args.dry_run:
